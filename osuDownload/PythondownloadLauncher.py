...
def download(url: str, file_name: str) -> None:
    ...
    # 一块文件的大小
    chunk_size = 1024
    with open(f'./beatmaps_xml/{file_name}', mode='wb') as f:
        # 写入分块文件
        for chunk in response.iter_content(chunk_size=chunk_size):
            f.write(chunk)


def pythonDownloadLauncher(fileName: str, fileNumber: int) -> None:
    ...
    for _ in tqdm(range(len(beatmap_id))):
        download(url=downLoadUrl[_], file_name=str(beatmap_id[_]) + '.osz')
        if _ % fileNumber == 0:
            multitasking.wait_for_tasks()

    end_time = time.time()
    print(f'耗时:{end_time - start_time},秒')
    print('下载结束，谱面已经保存在beatmaps_xml文件夹中')
